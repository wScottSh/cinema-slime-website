...
const DEFAULT_TIMEOUT = { maxWait: 6000, settleMs: 800 };
...
// Throws on invalid signature, wrong kind, a missing/mismatched coordinate,
// or malformed input — capture is refused loudly rather than storing a
// pointer to a body the site can't load.
function parseCaptureSource(source) {
  ...
  const coordinate = formatCoordinate({ kind: event.kind, pubkey: event.pubkey, identifier: identifier ?? '' });
  if (!coordinate) {
...
export function createEssayVault({ relayPort, store, readerRelays, writerRelays = readerRelays, timeout = DEFAULT_TIMEOUT }) {
  ...
  // always safe.
  function captureEssay(source) {
    const { event, coordinate } = parseCaptureSource(source);
  ...
  // a failure, never silently skipped.
  async function checkPresence(coordinates, relaysToRead) {
    ...
    for (const coordinate of coordinates) {
      ...
      const found = events.some((candidate) => candidate.id === stored.id && verifyEvent(candidate));
