...
import { runPresenceAudit, runRelayCoverageAudit } from '../scripts/check-curation.mjs';
...
// ─── runRelayCoverageAudit — per-relay Curation redundancy (#168) ──────────
//
// Reports, per brand relay, whether that relay holds the live Curation, and
// fails when fewer than `minCoverage` do — the check ADR 0014's open question
// (the Curation was found on only one relay during the July outage) asked
// for. `queryRelay` is injected so this is driven deterministically without
// any real relay I/O (ADR 0002).
