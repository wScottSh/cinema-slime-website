...
**Accepted trade-off (option a)**: if an Official Essay exists on no reachable relay, a cold Discovery load waits the full `maxWait` before rendering what it has, instead of a shorter incomplete-case cap or progressive rendering. Returning visitors are unaffected (SWR cache, ADR 0006). This is deliberate — the alternative (a shorter cap for the incomplete case) would silently reintroduce partial results, exactly the bug this ADR fixes. The live coverage check (`npm run check:coverage`, #169; see ADR 0017) makes chronic thin coverage visible so it's caught before this trade-off is exercised in practice — relay coverage is now a checked fact, not an assumption.
