...
- The paste-up treatment adds no image assets, and re-colours with the palette because it is filters and gradients over brand tokens. (Two brand images *do* change: `cs-logo.png` becomes the full logo sheet, and the old square disc mark is preserved as `cs-logo-sm.png`. Those are the brand mark itself, not texture.)
- **`/cs-logo.png` changed meaning**, and anything pointing at it inherited a wide banner where it used to get a square. The Essay Page's no-cover-image backdrop (`BRAND_MARK_URL` in `src/essay-header.js`) is the one such surface, and is repointed at `cs-logo-sm.png`. Any future square-mark surface — favicon, OG image — wants `cs-logo-sm.png`, not `cs-logo.png`.
